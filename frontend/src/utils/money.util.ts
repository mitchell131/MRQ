 ...
 */
export const formatAmount = ( amount: number, options?: Pick<IFormatMoneyOptions, 'noDecimal' | 'showDash'>, currency: string = '$' ) => {
  const { noDecimal, showDash = true } = options || {};

  let formatMoney = noDecimal ? Math.trunc( amount ) : parseFloat( ( amount / 100 ).toFixed( 2 ) );
